...
import DashboardView, { type BudgetData, type TransactionWithCategory } from './DashboardView';
...
type RecurringTransactionDB = { id: string; description: string; amount: number; type: 'income' | 'expense'; frequency: 'weekly' | 'monthly' | 'yearly'; day_of_month: number | null; day_of_week: number | null; start_date: string; end_date: string | null; category_id: string; categories: { id: string; name: string; icon: string | null; color: string | null; } | null; };
type ExpenseDB = { amount: number; categories: { name: string; color: string | null; } | null; [key: string]: any; };

const generateTransactionsFromRecurring = (recurringItems: RecurringTransactionDB[], startDate: Date, endDate: Date): any[] => {
  const generated: any[] = [];
  recurringItems.forEach(item => {
    const itemStartDate = new Date(item.start_date);
    const interval = { start: startDate > itemStartDate ? startDate : itemStartDate, end: item.end_date && new Date(item.end_date) < endDate ? new Date(item.end_date) : endDate, };
    ...
    eachDayOfInterval(interval).forEach(day => {
      ...
      if (shouldCreate) {
        generated.push({ id: `recurring-${item.id}-${format(day, 'yyyy-MM-dd')}`, description: item.description, amount: item.amount, date: format(day, 'yyyy-MM-dd'), type: item.type, category_id: item.category_id, categories: item.categories, is_recurring_instance: true, });
      }
    });
  });
  return generated;
...
export default async function DashboardPage() {
  ...
  const oneOffExpenses: ExpenseDB[] = (expensesResult.data as ExpenseDB[]) || [];
  ...
  const allMonthTransactions = [...oneOffIncomes.map(t => ({ ...t, type: 'income' })), ...oneOffExpenses.map(t => ({ ...t, type: 'expense' })), ...generatedRecurring];
  ...
  const expensesByCategory = allMonthTransactions.filter(t => t.type === 'expense').reduce<Record<string, { value: number; color: string }>>((acc, expense) => {
  ...
  const budgetsData: BudgetData[] = budgetedCategories.map(cat => {
      const spent = allMonthTransactions.filter(exp => exp.type === 'expense' && exp.category_id === cat.id).reduce((acc, exp) => acc + exp.amount, 0);
      return { id: cat.id, name: cat.name, icon: cat.icon, spent: spent, budget: cat.budget as number, progress: Math.min((spent / (cat.budget as number)) * 100, 100) };
  });

  const recentTransactions = allMonthTransactions.sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime()).slice(0, 5);
